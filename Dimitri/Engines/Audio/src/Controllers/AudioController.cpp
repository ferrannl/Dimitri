...
void Controllers::AudioController::add_sound(const std::string& name, const std::string& path)
{
	try {
		if (!name_exists(name)) {
			try {
				_audios.push_back(std::make_shared<Models::Audio>(name, std::make_unique<Facades::SoundFacade>(path, _channel_counter)));
				_channel_counter++;
			}
...
void Controllers::AudioController::add_music(const std::string& name, const std::string& path)
{
	try {
		if (!name_exists(name)) {
			try {
				_audios.push_back(std::make_shared<Models::Audio>(name, std::make_unique<Facades::MusicFacade>(path)));
			}
			catch (Exceptions::LoadAudioFailed e) {
				std::cout << e.get() << ": " << name << std::endl;
			}
