...
WorldAdapter::WorldAdapter(float width, float height)
{
	...
	// bottom
	create_ground_body(0.0f, -1.0f, width, 1.0f);
	// top
	create_ground_body(0.0f, height, width, 1.0f);
	// left
	create_ground_body(-1.0f, 0.0f, 1.0f, height);
	// right
	create_ground_body(width, 0.0f, 1.0f, height);
}
...
void WorldAdapter::add_shape(Shape shape, float x, float y)
{
	b2BodyDef bodyDef;
