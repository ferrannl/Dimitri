...
namespace Game {
	...
	void Views::HighscoreView::init_textures()
	{
		...
		_textures.push_back(std::make_shared<Graphics::Models::Sprite>(0, 0, 0, window_height, window_width, 0, bg_path, Graphics::Enums::FlipEnum::NONE, true));

		int place_margin = 0;

		//show all record times
		_textures.push_back(std::make_shared<Graphics::Models::Text>("Plaats", color, window_width / 4 + 100, (window_height - 170), 1, 40, 100, 0, path, true));
		_textures.push_back(std::make_shared<Graphics::Models::Text>("Tijden", color, window_width / 4 + 300, (window_height - 170), 1, 40, 100, 0, path, true));
		for (int i = 1; i <= 10; i++) {
			std::string place = std::to_string(i);
			_textures.push_back(std::make_shared<Graphics::Models::Text>(place, color, window_width / 4 + 100, (window_height - 225) - place_margin, 1, 40, 40, 0, path, true));
			place_margin = place_margin + 48;
		}
		place_margin = 0;
		for (int i = 0; i < _record_times.size(); i++) {
			std::string time = std::to_string(_record_times[i]);
			_textures.push_back(std::make_shared<Graphics::Models::Text>(time, color, window_width /4 + 300, (window_height - 225) - place_margin, 1, 40, 40, 0, path, true));
			place_margin = place_margin + 48;
		}
