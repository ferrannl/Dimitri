...
namespace Game {
	...
	void Views::PauseLevelView::init_textures()
	{
		...
		_textures.push_back(std::make_shared<Graphics::Models::Text>("pause", color, 0, 0, 5000, win_text_height, win_text_width, 0, font_path, true));
	}
	...
	void Views::PauseLevelView::draw()
	{
		...
		for (auto& t : _textures) {
			...
			t->set_y(std::get<1>(_graphics_controller->get_level_size()) - std::get<1>(camera_pos) - window_height + (window_height / 2 - t->get_height() / 2));
		}
		View::draw();
	}
}