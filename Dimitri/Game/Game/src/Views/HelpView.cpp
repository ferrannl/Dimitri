...
namespace Game {
	...
	void Views::HelpView::init_textures()
	{
		...
		_textures.push_back(std::make_shared<Graphics::Models::Sprite>(0, 0, 0, bg_height, bg_width, 0, bg_path, Graphics::Enums::FlipEnum::NONE, true, Graphics::Models::Center{ 0, 0 }, true));
		...
		for (int i = 0; i < gameplay.size(); i++) {
			...
			_textures.push_back(std::make_shared<Graphics::Models::Text>(gameplay[i], color, x + 75, start_y - step * i, 10, 50, width, 0, path, true, Graphics::Models::Center{ 0, 0 }, true));
		}
	}
