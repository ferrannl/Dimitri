...
void Game::Controllers::HomeController::load_buttons(std::shared_ptr<Managers::LevelManager> level_manager, std::shared_ptr<Managers::HighscoreManager> highscore_manager)
{
	...
	_buttons.push_back(std::make_shared<Game::Models::Buttons::AdvertisementButton>((_scene_width / 2) - (_scene_width / 4), 20, 1, 200, _scene_width / 2, Game::Enums::DirectionEnum::NONE, _scene_height, level_manager->get_window_controller(), Graphics::Models::Center{ 0,0 }));
}
