...
namespace Game {
	...
	void Controllers::WindowController::create_window(int height, int width)
	{
		...
		_views.insert({ Enums::ViewEnum::SAVE_GAME, std::make_unique<Views::SaveGameView>(_graphics_controller) });
		...
		_views.insert({ Enums::ViewEnum::LEVEL_SELECTOR, std::make_unique<Views::LevelSelectorView>(_graphics_controller) });
