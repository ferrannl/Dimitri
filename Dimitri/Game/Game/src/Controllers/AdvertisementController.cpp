...
void Game::Controllers::AdvertisementController::load_advertisements()
{
	...
	{
		_advertisements.push_back(std::make_shared<Graphics::Models::Sprite>(w - (_scene_width / 4), 20, 1, h, w, 0, entry.path().string(), Graphics::Enums::FlipEnum::NONE, true, Graphics::Models::Center{ 0, 0 }, true));
	}
}
