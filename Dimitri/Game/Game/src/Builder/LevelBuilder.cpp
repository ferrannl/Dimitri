...
void Builder::LevelBuilder::build_background(std::shared_ptr<Models::Level>& level) {
	...
	while (x < level->get_width()) {
		level->add_background(_backgroundFactory.create(Enums::TypeEnum::BG, x, y, 0, bg_height, bg_width, Enums::DirectionEnum::NONE));

		y += bg_height;

		while (y <= level->get_height()) {
			level->add_background(_backgroundFactory.create(Enums::TypeEnum::BG_TOP1, x, y,0, top_height, top_width, Enums::DirectionEnum::NONE));
			y += top_height;

			level->add_background(_backgroundFactory.create(Enums::TypeEnum::BG_TOP2, x, y,0, top_height,top_width, Enums::DirectionEnum::NONE));
			y += top_height;
		}
