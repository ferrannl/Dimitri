...
public:
	...
	std::vector<std::unique_ptr<Models::Shape>> shapes;
	PhysicsCollisionDemo();
	void start_demo();
	int create_window(int width, int height);
	void create_sprite(int x, int y, int z, int height, int width, std::string path, const float angle, Graphics::Enums::FlipEnum flipstatus, bool is_dynamic);
